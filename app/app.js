...
function decodeAttributesSerialize(serialized) {
	if (typeof serialized !== "string" || serialized.length < 4) return {};

	const bytes = new Uint8Array(serialized.length);
	for (let i = 0; i < serialized.length; i++) bytes[i] = serialized.charCodeAt(i) & 0xff;
	...
	const need = (n) => {
		if (offset + n > view.byteLength) throw new Error("Unexpected end of AttributesSerialize");
	};
	const u8 = () => { need(1); return view.getUint8(offset++); };
	const u16 = () => { need(2); const v = view.getUint16(offset, true); offset += 2; return v; };
	const u32 = () => { need(4); const v = view.getUint32(offset, true); offset += 4; return v; };
	const i32 = () => { need(4); const v = view.getInt32(offset, true); offset += 4; return v; };
	const f32 = () => { need(4); const v = view.getFloat32(offset, true); offset += 4; return v; };
	const f64 = () => { need(8); const v = view.getFloat64(offset, true); offset += 8; return v; };
	const string = () => {
		const length = u32();
		need(length);
	...
	const udim = () => ({ Scale: f32(), Offset: i32() });
	const udim2 = () => ({ X: udim(), Y: udim() });
	const color3 = () => ({ R: f32(), G: f32(), B: f32() });
	...
	const rotationMatrices = {
		...
		30:[0,0,-1,0,1,0,1,0,0],
		31:[0,0,-1,0,0,1,1,0,0],
		32:[0,0,1,0,1,0,1,0,0],
		16:[0,0,-1,0,0,-1,1,0,0],
		34:[0,0,1,0,0,1,-1,0,0],
		35:[0,0,-1,0,0,-1,-1,0,0]
	};
	...
	const cframe = () => {
		...
		if (rotationId === 0) {
			...
			for (let i = 0; i < 9; i++) Components.push(f32());
		} else {
			Components = [Position.X, Position.Y, Position.Z].concat(rotationMatrices[rotationId] || []);
		}
		return { Position, RotationId: rotationId, Components };
	};

	const readValue = (type) => {
		switch (type) {
			case 0x02: return string();
			case 0x03: return u8() !== 0;
			case 0x04: return i32();
			case 0x05: return f32();
			case 0x06: return f64();
			case 0x09: return udim();
			case 0x0a: return udim2();
			case 0x0e: return { Number: u32() };
			case 0x0f: return color3();
			case 0x10: return vector2();
			case 0x11: return vector3();
			case 0x14: return cframe();
			case 0x15: return { Enum: string(), Value: u32() };
			case 0x17: {
				const count = u32(), keypoints = [];
				for (let i = 0; i < count; i++) keypoints.push({ Envelope: f32(), Time: f32(), Value: f32() });
				return keypoints;
			}
			case 0x19: {
				const count = u32(), keypoints = [];
				for (let i = 0; i < count; i++) keypoints.push({ Envelope: f32(), Time: f32(), Value: color3() });
				return keypoints;
			}
			case 0x1b: return { Min: f32(), Max: f32() };
			case 0x1c: return { Min: vector2(), Max: vector2() };
			case 0x21: return { Weight: u16(), Style: u8(), Family: string(), CachedFaceId: string() };
			default:
				throw new Error("Unsupported attribute type 0x" + type.toString(16));
		}
	};
	...
	try {
		...
		for (let i = 0; i < count; i++) {
			const name = string();
			const type = u8();
			attributes[name] = readValue(type);
		}
		return attributes;
	} catch (error) {
		console.warn("Could not decode AttributesSerialize:", error);
		return {};
	}
}
...
async function start() {
	...
	const hiddenProperties = ["Children", "Source", "CollisionGroupData", "PhysicsGrid", "SmoothGrid", "MaterialColors", "AttributesSerialize"];
	...
	$('.tree').on("changed.jstree", async function (e, data) {
		...
		if (typeof instance.AttributesSerialize === "string") {
			const attributes = decodeAttributesSerialize(instance.AttributesSerialize);
			if (Object.keys(attributes).length > 0) instance.Attributes = attributes;
		}
		(async () => {
